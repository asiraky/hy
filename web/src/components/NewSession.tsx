...
import type { WorkspaceChoice } from "./WorkspacePicker";

export interface NewSessionInput { projectId: string; harness: string; model: string; branch: string; workspace: string; workspacePath: string }

export interface WorkspaceListing { workspaces: Workspace[]; issues: Issue[]; issuesError: string }

export function NewSession({ projects, harnesses, userConfig, onCreate, onListWorkspaces, onAddProject, onSettings, onRecheck, onClose, status }: {
  projects: Project[]; harnesses: HarnessMeta[]; userConfig: UserConfig | null; onCreate: (input: NewSessionInput) => Promise<void>;
  ...
  const [listing, setListing] = useState<WorkspaceListing>({ workspaces: [], issues: [], issuesError: "" });
  ...
  const workspace = choice.attachPath ? "" : branch ? "managed" : project?.config.defaults.workspace ?? "local";
  const ready = status === "online" && !!project && selected?.availability.state === "ready";
  const create = async () => { if (!project) return; setBusy(true); setError(null); try { await onCreate({projectId:project.id,harness:harnessId,model,branch,workspace,workspacePath:choice.attachPath}); } catch(e) { setError(e instanceof Error ? e.message : String(e)); setBusy(false); } };
  ...
  useEffect(() => {
    ...
    onListWorkspaces(project.id)
      .then(r => { if (live) setListing(r); })
      .catch(e => { if (live) setListing({ workspaces: [], issues: [], issuesError: e instanceof Error ? e.message : String(e) }); })
      .finally(() => { if (live) setLoadingSpaces(false); });
    return () => { live = false; };
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onClick={onClose}>
    <div className="scroll-thin fade-in max-h-[85dvh] w-full max-w-md overflow-y-auto rounded-2xl border border-ink-800 bg-ink-900 p-5 shadow-2xl" onClick={e=>e.stopPropagation()}>
      ...
      {projects.length === 0 ? <div className="mt-5 rounded-xl border border-dashed border-ink-700 p-5 text-center"><p className="text-[13px] text-ink-300">Add a project once, then create every session from here.</p><Button variant="primary" className="mt-3" onClick={onAddProject}>Add project</Button></div> : <>
        ...
        <label className="mt-4 block text-[11px] uppercase tracking-wide text-ink-500">Workspace</label>
        <WorkspacePicker value={choice} onChange={setChoice} workspaces={listing.workspaces} issues={listing.issues} issuesError={listing.issuesError} userConfig={userConfig} loading={loadingSpaces} placeholder="issue/482-fix-login"/>
      </>}
